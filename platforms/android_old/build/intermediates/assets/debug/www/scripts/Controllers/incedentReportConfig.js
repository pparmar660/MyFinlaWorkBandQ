        ...
        $scope.backPage = function (page) {
            ...
            setTimeout(function () {
                $scope.$apply(function () {
                    $scope.setFieldSetOnNextAndBackInDropDown($scope.pageArray[index - noOfPageMove]);
                    var currentPage = $scope.pageArray[index - noOfPageMove]
                    $rootScope.currentPagesScopeArray = [];
                    for (var j = 0; j < currentPage.fields.length; j++)
                        $rootScope.currentPagesScopeArray.push(angular.element($('#' + currentPage.fields[j])).scope())
