...
BandQModule.controller("ViewVechileSecurityById", ['$scope', '$rootScope',
    'checkInternetConnectionService', 'imageService', 'TaskAndCheckList', '$timeout',
    'getAllFieldLabel', 'linkedModule', function ($scope, $rootScope, checkInternetConnectionService, imageService, TaskAndCheckList, $timeout, getAllFieldLabel, linkedModule) {
...
//                    null, constanObject.CommsAndTaskWebRequestType.CommsDetails, true);
            ...
            TaskAndCheckList.getTaskList("Vehicle", vehicle_id, function (status, data) {
                $scope.$apply(function () {
                    if (status) {
                        ...
                        $('#vehTasksTab').removeClass("hideTab");
                    }
                    else {
                        $scope.showChecklist = false;
                        $('#vehTasksTab').addClass("hideTab");
                    }
        ...
        function setFirstTab() {
            ...
            //$("#vehicleDetails").find(".parentHorizontalTab ul.hor_1 li.resp-tab-active").parent().siblings().find('li').removeClass("resp-tab-active");
...
//            $(".someTab #in_tab1").show();
//            $(".someTab #in_tab1").siblings().hide();
            if ($(window).width() < 768) {
 ...
 $scope.$on('myTabSelectEvent', function (event, data) {
        ...
        function handleLinkedNavigation(data) {
            if (!data.length > 0)
            {
                document.getElementById("linkedMapVehicleDetailOutSide").style.display = "none";
                return;
            }
            linkedLocationData = data;//,
            document.getElementById("linkedMapVehicleDetailOutSide").style.display = "block";
            var mapDiv = document.getElementById("linkedMapVehicleDetailOutSide");
                    ...
                    if (status == constanObject.SUCCESS) {
                        ...
                        $scope.$apply(function () {
                            ...
                            if (responseData.data.vehicle_detail[0].add_comm == 1) {
                                ...
                                getCommsDetails(vehicle_id);
                                 $('#vehCommsTab').removeClass("hideTab");
                            } else {
                                $scope.isCommAllowed = false;
                                 $('#vehCommsTab').addClass("hideTab");
                            }
                            ...
                            if (data.linked_locations.length > 0) {
                                $scope.showmap = true;
                                $('#vehLocationTab').removeClass("hideTab");
                                handleLinkedNavigation(data.linked_locations);
                            } else {
                                $scope.showmap = false;
                                $('#vehLocationTab').addClass("hideTab");
                            }
                        ...
                        if ($scope.data) {
                            if ($scope.data.linked_offender.length > 0 || $scope.data.incident.length > 0 || $scope.data.linked_locations.length > 0) {
                                $scope.showVehLinkedDetailsTab = true;
                                if ($scope.data.linked_offender.length > 0)
                                    $('#vehLinkedOffenderTab').removeClass("hideTab");
                                else
                                    $('#vehLinkedOffenderTab').addClass("hideTab");
                                if ($scope.data.incident.length > 0)
                                    $('#vehIncidentTab').removeClass("hideTab");
                                else
                                    $('#vehIncidentTab').addClass("hideTab");

                            } else {
                                $scope.showVehLinkedDetailsTab = false;
                            }
