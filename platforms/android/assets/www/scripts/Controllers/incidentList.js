...
//        if ($rootScope.victimListShowStatus) {
    ...
    $scope.preRequest = function () {
        if ($scope.page > 1) {
            ...
            window.scrollTo(0, 0);
            IncidentReport.getIncidentReportList($scope.incidentURL, $scope.listType, $scope.page, searchJSON, 
            function (status, data, count,lastPage, list_type) {
              //  (true, responseData.data, responseData.count, responseData.last_page,responseData.list_type
                $scope.lastPage = lastPage;
                if (status) {

                    if (list_type == "draft")
                        $scope.isListTypeDraft = true;
                    else
                        $scope.isListTypeDraft = false;
    ...
    $scope.getList = function (parameter) {
        ...
        $scope.incidentURL = constanObject.INCIDENT_REPORT_LIST_BY_STATUS + $scope.incidentStatus;
        IncidentReport.getIncidentReportList($scope.incidentURL, $scope.listType, $scope.page, parameter,
        function (status, data, count, lastPage, list_type) {
            $scope.lastPage = lastPage;
            if (status) {
...
//        $("#dashboardTab").removeClass("active");
    ...
    $scope.webRequestResponse = function (requestType, status, responseData) {
        ...
        if (status == constanObject.ERROR) {
            if (requestType == constanObject.WebRequestType.DELETE_DRAFT_INCIDENT) {
                $scope.deletIncidentAlertPopup = false;
        ...
        switch (requestType) {
            ...
            case constanObject.WebRequestType.DELETE_DRAFT_INCIDENT:
                $scope.deletIncidentAlertPopup = false;
                $scope.$apply(function () {
                    $scope.IncidentList.splice(positionSelectedIncidentForDelete, 1);
    ...
    var idSelectedIncidentForDelete;
    var positionSelectedIncidentForDelete;
    $scope.deleteDraftReport = function (id, position, referenceId) {
        positionSelectedIncidentForDelete = position;
        $scope.deletIncidentAlertPopup = true;
        idSelectedIncidentForDelete = id;
        $scope.deleteIncident= referenceId;

    }
    $scope.yesAction = function () {
        var url = constanObject.DELETE_DRAFT_INCIDENT + idSelectedIncidentForDelete;
        webRequestObject.postRequestJSON($scope, "DELETE", url, "", constanObject.WebRequestType.DELETE_DRAFT_INCIDENT, true);

    }

    $scope.noAction = function () {
        $scope.deletIncidentAlertPopup = false;
        idSelectedIncidentForDelete = "";
    }
