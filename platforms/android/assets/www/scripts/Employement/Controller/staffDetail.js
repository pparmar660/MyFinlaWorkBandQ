BandQModule.controller("staffDetail", ['$scope', '$rootScope', 'checkInternetConnectionService', 'globalService', 'appMenuConfig', 'moduleAccessPermission', 'imageService', function ($scope, $rootScope, checkInternetConnectionService, globalService, appMenuConfig, moduleAccessPermission, imageService) {
        $scope.staffProfile = [];
        ...
        $scope.moduleAccessInStaffDetail = {};
        $scope.moduleAccessInStaffDetail = moduleAccessPermission.getModulePermission();
        var SECURITY = 38,
                COMMSTASK = 36;
        $scope.menu = {};
...
//       
//              newstaffDetailScope.stafImage = constanObject.GET_STAFF_IMAGE + $scope.staffProfile[0].id_usr + "/5";
//               $scope.stafImage = constanObject.GET_STAFF_IMAGE + $scope.staffProfile[0].id_usr + "/5";
//         
...
//date of birth format
            ...
            $scope.stafImage = constanObject.GET_STAFF_IMAGE + $scope.staffProfile[0].id_usr + "/1";
              $rootScope.$broadcast('profileImage', {image: $scope.stafImage});
        ...
        $scope.saveButton = function () {
            if ($scope.profileImage)
                webRequestObject.fileUpload($scope, $scope.profileImage.url, constanObject.UPLOAD_STAFF_IMAGE + $scope.staffId, "image_users", constanObject.FileUploadModuleId.STAFF, true);
        ...
        $scope.webRequestResponse = function (requestType, status, response) {
            ...
            switch (requestType) {
                case constanObject.employmentWebRequestType.STAFF_DETAIL:
                    if ($rootScope.isEditProfile) {
                        ...
                        $scope.$apply(function () {
                            ...
                            $scope.stafImage = constanObject.GET_STAFF_IMAGE + response[0].id_usr + "/1";
                        ...
                        $scope.$apply(function () {
                            ...
                            $scope.stafImage = constanObject.GET_STAFF_IMAGE + response[0].id_usr + "/1";
                            $scope.staffImages = response[0].linked_images;
                            $scope.getProfile();
                            $rootScope.staffDirectorytemplate = false;
                ...
                case constanObject.FileUploadModuleId.STAFF:
                    //  console.log("update image : " + JSON.stringify(response));

                    if (status) {
                        $scope.showSaveButton = false;
                        $scope.stafImage = null;
