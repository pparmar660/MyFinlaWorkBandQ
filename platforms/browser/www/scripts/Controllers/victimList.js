BandQModule.controller('victimwinessCnt', function ($scope, VictimWitness, $rootScope, 
checkInternetConnectionService, globalService) {
    $scope.categoryList = [];
    ...
    $scope.init = function () {
        ...
        $scope.infoShow1 = false;
        $scope.categoryList = VictimWitness.getVictimCategory().category;
        $scope.page = 1;
        for (var i = 0; i < $scope.categoryList.length; i++) {
            if ($scope.categoryList[i].selected)
                $scope.selectedCategory.push($scope.categoryList[i].id_uct);
        }
...
//        scope.init(obj.id_usr);
    ...
    $scope.removeVictim = function (obj) {
        ...
        $scope.selectedVictims = $.grep($scope.selectedVictims, function (e) {
    ...
    $scope.categoryChangeAction = function (index, catObj) {
        if ($scope.categoryList[index].selected == true) {
    ...
    $scope.previousPageRequest = function () {
        ...
        if ($scope.page > 1) {
            ...
            var filterData = {search: $scope.filterParam};
            VictimWitness.getVictimeData(page, $scope.url, filterData, function (status, response, data, next_page_url, prev_page_url) {
                $scope.$apply(function () {
                    if (page == 1) {
                        $scope.prevShowStatus = false;
                        $scope.nextShowStatus = true;
...
//                console.log("$scope.selectedVictims :"+JSON.stringify($scope.selectedVictims));
                VictimWitness.setVictimCategory({'category': $scope.categoryList, 'selectedVictimCategory': $scope.selectedCategory});
...
//        return callback(false, 0);
    ...
    $scope.getVictimWitnessList = function () {
        ...
        VictimWitness.getVictimeData($scope.page, $scope.url, searchData, function (status, response, data, next_page_url, prev_page_url) {
            $scope.$apply(function () {
                if (!status) {
                    $scope.infoShow1 = true;
                    $scope.prevShowStatus = false;
...
//    };
    ...
    $scope.reset = function () {
        $scope.selectedCategory = [];
        if (!globalService.getVictim().victimDetails)
            return;
        if (globalService.getVictim().victimDetails) {
