BandQModule.controller("doYouHaveAnyFileController", ['$scope', '$timeout',
    'globalService', 'imageService', 'getUniqueId',
    function ($scope, $timeout, globalService, imageService, getUniqueId) {
        ...
        $scope.spliceItem = function (data, ev) {
        ...
        $scope.nextButtonClicked = function () {
            ...
            uploadFileUp();
            globalService.setDoYouHaveFiles($scope.items);
     ...
     $scope.saveButtonClicked = function (callBack){
                ...
                case 1:
                    ...
                    imageService.getMediaImage(function (item) {
                        $scope.$apply(function () {
                            ...
                            onlinePacket = [constanObject.FileUploadModuleId.FILE_UPLOAD.toString(), 
                                item.id.toString(), constanObject.CREATE_INCIDEN_TEMP_ID.toString(),
                                item.src.toString(), "image", "0", item.id.toString(), globalService.getUserId().toString(), "-1"];
                    ...
                    imageService.getVideo(function (item) {
                        $timeout(function () {
                            imageService.createThumb(item, function (i2) {
                                $scope.$apply(function () {
                                    i2.id = getUniqueId.getId();
                                    $scope.items.push(i2);
                                    onlinePacket = [constanObject.FileUploadModuleId.FILE_UPLOAD.toString(),
                                        i2.id.toString(), constanObject.CREATE_INCIDEN_TEMP_ID.toString(),
                                        i2.src.toString(), "image", "0", item.id.toString(), globalService.getUserId().toString(), 
                                        "-1"];
                                    ...
                                    dataBaseObj.insertData(
                                        ...
                                        globalService.setFileUploadsTempIds( i2.id);
        ...
        $scope.reset = function () {
            var data = globalService.getDoYouHaveFiles();
            if (data)
            if (data.length > 0){
            $scope.items = data;
             $scope.showUpload=1;
