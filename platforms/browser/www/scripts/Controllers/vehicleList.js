...
var vehicle_id = 0;
BandQModule.controller("ViewVechiles", function ($scope, $rootScope, globalService, vehicleService, imageService, checkInternetConnectionService) {
...
//    $scope.fullVehicleCount = 0;
    ...
    $scope.init = function () {
        ...
        var vehicleList = angular.copy(vehicleService.getVehicleList().data);
        $scope.search.name = globalService.getInvolvedVehicleDetails().licence_plate_vtk;
        $scope.vehicleCount = vehicleService.getVehicleList().category_count;
        $scope.isFullPlate = globalService.getInvolvedVehicleDetails().licence_plate_type;
        $scope.activeMenu = globalService.getInvolvedVehicleDetails().licence_plate_type;
        if ($scope.isFullPlate == 1)
            $scope.url = constanObject.vechileFullPlate
        else
            $scope.url = constanObject.vechilePartial;
        if (vehicleService.getVehicleList().next_page_url) {
    ...
    $scope.nextPage = function () {
        ...
        $scope.page++;
        if ($scope.pagingNext) {
            var searchData = {search: $scope.search};
            webRequestObject.postRequest($scope, "GET", $scope.url + "?page=" + $scope.page, searchData, constanObject.WebRequestType.VechileSearch, false);
            window.scrollTo(0, 0);
        }
    };
    ...
    $scope.prevPage = function () {
        ...
        if ($scope.vechilePrevURL != null) {
            $scope.page = ($scope.page > 1 ? (--$scope.page) : 1);
            var searchData = {search: $scope.search};
            webRequestObject.postRequest($scope, "GET", $scope.url + "?page=" + $scope.page, searchData, constanObject.WebRequestType.VechileSearch, false);
    ...
    $scope.getVechileDetail = function (vechileId) {
        ...
        $rootScope.vehicleId = vechileId;
        $rootScope.isEditVechile = false;
        $rootScope.show = false;
        $rootScope.vehicleAddShowStatus = false;
        $rootScope.vehicleDetailShowStatus = true;
        $rootScope.vehicleListShowStatus = false;
        var scope = angular.element('#208').scope();
        scope.init();
    };

    $scope.getVechiles = function () {
        $scope.plateSearchVal = $scope.search.name;
        var search = {search: $scope.search};
        webRequestObject.postRequest($scope, "GET", $scope.url + "?page=" + $scope.page, search, constanObject.WebRequestType.VechileSearch, false);
    };
    ...
    $scope.nextButtonClicked = function (callBack) {
        ...
        if ($rootScope.vehicleListShowStatus) {
            ...
            try {
                $scope.VechilesList = globalService.getVehicle();
                globalService.setInvolvedVehicleDetails({vehicleList: $scope.VechilesList, licence_plate_vtk: $scope.plateSearchVal, licence_plate_type: $scope.isFullPlate, areVehicleInvolved: "yes"});
    ...
    $scope.searchVechileTab = function (tab) {
        ...
        if (tab == 0) {
            ...
            $("#tab_all").removeClass("active");
            webRequestObject.postRequest($scope, "GET", $scope.url + "?page=" + $scope.page, search, constanObject.WebRequestType.VechileSearch, false);
            newVechileScope.VechileDetails.licence_plate_type = 2;
        }
    ...
    $scope.reset = function () {
        $scope.VechilesList = '';
        $scope.activeMenu = globalService.getInvolvedVehicleDetails().licence_plate_type;
        if (globalService.getInvolvedVehicleDetails().vehicleList) {
            $scope.VechilesList = globalService.getInvolvedVehicleDetails().vehicleList;
            //globalService.addVehicle($scope.VechilesList);
