BandQModule.controller('offenderDetailCtrl', function ($scope, OffenderService, $rootScope,
        imageService, TaskAndCheckList, checkInternetConnectionService, getAllFieldLabel, linkedModule) {
    $scope.offender = null;
    ...
    $scope.getOffenderDetail = function (offender_id) {
        ...
        TaskAndCheckList.getTaskList("Offender", offender_id, function (status, data) {
            if (status) {
                ...
                $('#offTasksTab').removeClass("hideTab");
            }
            else {
                $scope.showchecklist = false;
                $('#offTasksTab').addClass("hideTab");
            }
            $scope.$apply(function () {
...
//                obj.model_vtk==null?obj.model_vtk="":obj.model_vtk=obj.model_vtk;
            ...
            if (data.linked_locations.length > 0) {
                $scope.showMap = true;
                $('#offLocationTab').removeClass("hideTab");
                handleLinkedNavigation(data.linked_locations);

            } else {
                $scope.showMap = false;
                $('#offLocationTab').addClass("hideTab");
            }
            ...
            $scope.$apply(function () {
                ...
                if (offenderData.add_comm == 1) {
                    ...
                    $('#offCommsTab').removeClass("hideTab");
                } else {
                    $scope.isCommAllowed = false;
                    $('#offCommsTab').addClass("hideTab");
                }
...
//                }
            ...
            if ($scope.data) {
                if ($scope.data.linked_offender.length > 0 || $scope.data.vehicle.length > 0 || $scope.data.incident.length > 0 || $scope.data.linked_locations.length > 0) {
                    $scope.showOffLinkedDetailsTab = true;
                    if ($scope.data.linked_offender.length > 0)
                        $('#offLinkedOffenderTab').removeClass("hideTab");
                    else
                        $('#offLinkedOffenderTab').addClass("hideTab");
                    if ($scope.data.vehicle.length > 0)
                        $('#offVehicleTab').removeClass("hideTab");
                    else
                        $('#offVehicleTab').addClass("hideTab");
                    if ($scope.data.incident.length > 0)
                        $('#offIncidentTab').removeClass("hideTab");
                    else
                        $('#offIncidentTab').addClass("hideTab");

                } else {
                    $scope.showOffLinkedDetailsTab = false;
                }
