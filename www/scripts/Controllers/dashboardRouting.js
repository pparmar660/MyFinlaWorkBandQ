var angular;
BandQModule.controller("dashboardCtrl", ['$rootScope', '$scope', 'checkInternetConnectionService',
    'appMenuConfig', '$timeout', 'moduleAccessPermission',
    "globalService", "manualSync",
    function ($rootScope, $scope, checkInternetConnectionService, appMenuConfig,
            $timeout, moduleAccessPermission, globalService, manualSync) {
        $rootScope.highlightedFooter = 'Dashboard';
        $rootScope.moduleIconClass = 'dahboard-icon';
        $rootScope.moveToPage = '#/dashboard';
...
//            }
    ...
    $scope.loadConfigData();
        ...
        $scope.viewCommsTask = function () {
            ...
            $rootScope.highlightedFooter = 'Comms & Tasks';
            $rootScope.moveToPage = '#/commsTasks';
            $rootScope.moduleIconClass = 'calender-icon';
            $rootScope.subMenuTitle1 = '';
            $rootScope.dashboardLink = '#/dashboard';
...
//        };
        ...
        $scope.callResourceModule = function (module) {
            window.location.href = "dashboard.html#/resource";
            $rootScope.highlightedFooter = 'Resources';
            $rootScope.moveToPage = '#/resource';
            $rootScope.moduleIconClass = 'resources-icon';
            ...
            setTimeout(function () {
                ...
                if (module == "document")
                    appObj.load_listing_tab($('ul li a[data-tab="document"]'));

                if (module == "video")
                    appObj.load_listing_tab($('ul li a[data-tab="video"]'));

                if (module == "link")
                    appObj.load_listing_tab($('ul li a[data-tab="link"]'));

                if (module == "news")
                    appObj.load_listing_tab($('ul li a[data-tab="news"]'));

                if (module == "faq")
                    appObj.load_listing_tab($('ul li a[data-tab="faq"]'));
        ...
        var autoManulSyncForFirstTime = function () {


            if (manualSync) {
                manualSync.AfterLoginPageupdateData();
            } else
                setTimeout(function () {
                    autoManulSyncForFirstTime();
                }, 100);
...
BandQModule.controller("resourceCtrl", ['$scope', '$rootScope', function ($scope, $rootScope) {
        $rootScope.highlightedFooter = 'Resources';
        $rootScope.moveToPage = '#/resource';
        $rootScope.moduleIconClass = 'resources-icon';
        $rootScope.menuTitle = 'Resources';
