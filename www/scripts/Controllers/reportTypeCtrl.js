BandQModule.controller("ReportTypeCtrl", function ($scope, $http, $rootScope, CategoryOutcomeTypeService, globalService, showHidePageService, dynamicQuestion) {
                    ...
                    if (targetOutcomeId == allOutcomes[c].id_otc) {
                        ...
                        for (var so = 0; so < outTypeLimit; so++) {
                            ...
                            if (parseInt(userDefaultTypeIds[so]) == parseInt(allTypes[o].id_it)) {
                                    ...
                                    if ((allTypes[o].swas_area_check == 0) || (allTypes[o].assigned_swas.indexOf(globalService.getSwasAreaId() +"")>=0) &&
                                        (allTypes[o].assigned_sectors_check == 0) || (allTypes[o].assigned_sectors.indexOf(globalService.getSectorId() +"")>=0)
                                            && (allTypes[o].assigned_levels_check == 0) || (allTypes[o].assigned_levels.indexOf(User_Access_Leve) +"")>=0) {
            ...
            $scope.changeOutcome = function (obj) {
                ...
                if (obj.id_it != globalService.getTypeId()) {
                    var empty = [];
                ...
                globalService.setTypeObj(obj.id_it);
            ...
            $scope.reset = function () {
                ...
                if (globalService.getTypeObj())
                {
                    var selectedType = globalService.getTypeObj();
                    for (var i = 0; i < $scope.ts.length; i++) {
                        if (selectedType == $scope.ts[i].id_it) {
                            selectType($scope.ts[i]);
                            $scope.hasSelectedType = true;
                        }
                    }
                    if ($scope.hasSelectedType) {
                        globalService.setTypeId(selectedType);
                        globalService.setTypeObj(selectedType);
                        chekQuestionExist();

                    } else {
                        globalService.setTypeId(-1);
                        globalService.setTypeObj(-1);
                    }
